...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Convert query params to single string
    ...
    if (searchQuery && searchQuery.trim() !== '') {
      backendUrl =
        `${apiBaseUrl}/searchJobStatus?searchQuery=${encodeURIComponent(searchQuery)}` +
      ...
      backendUrl =
        `${apiBaseUrl}/fetchJobStatus?roleType=${encodeURIComponent(roleType)}` +
    ...
    console.error('Error in jobStatus API:', error);
