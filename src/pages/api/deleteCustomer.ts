...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const { customerId } = req.body; // Expecting customerId from the request body
    const token = req.headers.authorization?.split(' ')[1]; // Extract the token from Authorization header

    if (!token || !customerId) {
      return res.status(400).json({ message: 'Missing token or customerId' });
    }

    // Make the necessary backend call to delete the technician (in your DB, etc.)
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/deleteCustomer`, {
      method: 'POST',
      headers: {
