...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const backendResponse = await fetch(
      `${apiBaseUrl}/fetchCustomerJobs?customerId=${encodeURIComponent(customerId)}&page=${encodeURIComponent(page)}&limit=${encodeURIComponent(limit)}`,
      {
        method: 'GET',
        headers: {
    ...
    console.error('Error in fetchCustomerJobs API:', error);
