...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const backendResponse = await fetch(
      `${apiBaseUrl}/fetchJobVehicles?jobId=${encodeURIComponent(jobId)}&page=${encodeURIComponent(page)}&limit=${encodeURIComponent(limit)}`,
      {
        method: 'GET',
        headers: {
    ...
    console.error('Error in fetchJobVehicles API:', error);
