...
export default function ViewDetails() {
  ...
  const fetchJobVehicles = async (jobId: string, page = 1) => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_PATH}/api/fetchJobVehicles?jobId=${encodeURIComponent(jobId)}&page=${page}&limit=${VEHICLE_LIST_PAGE_SIZE}`,
        { method: 'GET', headers }
      );
  ...
  const markRowsAsPaid = async (rows: TechnicianVehicleAssignmentRow[]) => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_PATH}/api/markVehicleTechnicianPaid`, {
        method: 'POST',
        headers,
        body: JSON.stringify({
          jobId,
    ...
    <div className={`admin-view-detail mobile_listing mx-auto mt-4 transition-all duration-300 ${isCollapsed ? 'w-full pl-[5rem]' : 'w-full lg:container'}`}>
      ...
      <div className="mx-auto">
        ...
        {/* <div className="flex items-center gap-3 mb-4">
          <Link href="/admin/jobs/active-job" className="flex items-center gap-2 hover:opacity-90 transition-opacity">
