...
//   const fetchCustomerData = async (vin: string, customerId: string, jobId: string, filterType: string = '') => {
...
//         `/api/viewGroupJobByVin?roleType=${roleType}&vin=${vin}&customerId=${customerId}&jobId=${jobId}&filterType=${filterType}&userId=${userId}`,
//         {
//           method: "GET",
//           headers,
//         }
...
//       setJobId(jobId);
//       fetchCustomerData(vin, customerId, jobId);
//     } else {
//       setIsEdit(false);
//     }
//   }, []);
...
//                   fetchCustomerData(vin, customerId, jobId, 'completed');
...
//                   fetchCustomerData(vin, customerId, jobId, 'inProgress');
//                 }}>
//                 In Progress Jobs
//               </button>
//               <button className={`text-xs border border-gray-300 p-3 pl-4 pr-4 rounded 
...
//                   fetchCustomerData(vin, customerId, jobId, 'missingPayRates');
//                 }}>
//                 Pay Rates Missing
//               </button>
...
//                   </h2>
//                   <Link className="p-2" href={`/admin/jobs/create-job/create?jobId=${job.id}&groupjob`} data-tooltip-id="edit"
...
export default function ViewDetails() {
  const [jobData, setJobsData] = useState<any>(null); // Changed to single object
  const [isEdit, setIsEdit] = useState<boolean>(false);
  const [userType, setUserType] = useState<string | null>(null);
  const [previewImage, setPreviewImage] = useState<string | null>(null);
  const [filterType, setFilterType] = useState<'' | 'completed' | 'inProgress' | 'missingPayRates'>('');
  ...
  const calculateTechnicianTotalCost = (jobData: any, technician: any) => {
  if (!jobData || !technician) return 0;
  ...
  const techDetails = technician.VehicleTechnician;

  // Parse simpleFlatRate if available
  let simpleFlatRate = 0;
  try {
    const parsedRate = techDetails?.simpleFlatRate ? JSON.parse(techDetails.simpleFlatRate) : null;
    simpleFlatRate = parsedRate?.[techDetails.payVehicleType] || 0; // Get specific rate for the selected vehicle type
  } catch (error) {
    console.error('Error parsing simpleFlatRate:', error);
    simpleFlatRate = 0;
  }

  // Parse amountPercentage
  const amountPercentage = parseFloat(techDetails?.amountPercentage || '0');

  let technicianTotal = subtotalcost;

  // Add simpleFlatRate if available
  if (simpleFlatRate > 0) {
    technicianTotal += simpleFlatRate;
  }
  // Otherwise, apply percentage if available
  ...
  if (!jobData.jobs || jobData.jobs.length === 0) {
    return (
      <>
        <div className="w-[80%] ml-auto p-4 laptop-narrow">
          <div className='flex items-center gap-4 justify-end mb-5 bg-white p-3 sticky top-[4.8rem]'>
            {/* <div className="flex w-[300px] relative search__input">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" style={{ position: 'absolute', right: '10px', top: '12px', zIndex: '1' }} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <circle cx="11" cy="11" r="8"></circle>
                <line x1="21" y1="21" x2="16.65" y2="16.65"></line>
    ...
    <>
      <div>
        <div className="w-[80%] ml-auto p-4 laptop-narrow">
          ...
          <div className="rounded-lg">
            {jobData.vehicles.map((job: any, index: number) => (
              <div key={job?.id} className="bg-blue rounded-lg shadow-md mb-6">
                <div className="flex justify-between items-center pb-3 pl-6 pr-6 pt-4">
                  <h2 className="text-xl font-bold">
                    Work Order Id - {job?.jobId}
                  </h2>
                  <Link className="p-2" href={`/admin/vehicle/create-vehicle?vahicleId=${job.id}&groupjob`} data-tooltip-id="edit"
                    data-tooltip-content="Edit">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M11 4H4C3.46957 4 2.96086 4.21071 2.58579 4.58579C2.21071 4.96086 2 5.46957 2 6V20C2 20.5304 2.21071 21.0391 2.58579 21.4142C2.96086 21.7893 3.46957 22 4 22H18C18.5304 22 19.0391 21.7893 19.4142 21.4142C19.7893 21.0391 20 20.5304 20 20V13" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                      <path d="M18.5 2.50023C18.8978 2.1024 19.4374 1.87891 20 1.87891C20.5626 1.87891 21.1022 2.1024 21.5 2.50023C21.8978 2.89805 22.1213 3.43762 22.1213 4.00023C22.1213 4.56284 21.8978 5.1024 21.5 5.50023L12 15.0002L8 16.0002L9 12.0002L18.5 2.50023Z" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                ...
                {job.assignedTechnicians?.map((tech: any, index: number) => (
                  <div>
                    <div className="flex items-center gap-3 pl-6 pr-2 pt-4 bg-white">
                      {tech.image ? (
                        <img
                        ...
                        <div className="w-[60px] h-[60px] rounded-full bg-[#383D71] flex items-center justify-center text-white font-bold text-2xl">
                          {tech.firstName?.charAt(0)?.toUpperCase() || 'T'}
                        </div>
                      )}
                    ...
                    <div className="grid grid-cols-2 pl-6 pr-2 pb-3 pt-4 bg-white">
                      ...
                      <div className="pl-2 pr-2">
                        {userType !== 'single-technician' && (
                          <div className="mb-2 text-sm items-center flex">
                            ...
                            {(() => {
                              ...
                              // Calculate total cost from job description if available
                              if (job?.jobDescription && Array.isArray(job.jobDescription)) {
                                totalCost = job.jobDescription.reduce((sum: number, item: any) => {
                              ...
                              // Parse simpleFlatRate for this technician
                              const simpleFlatRateData = tech?.VehicleTechnician?.simpleFlatRate;
                              let simpleFlatRate = 0;

                              // Try parsing the simpleFlatRate JSON string for each technician
                              try {
                                const parsedData = simpleFlatRateData ? JSON.parse(simpleFlatRateData) : null;
                                if (parsedData && parsedData[tech?.VehicleTechnician?.payVehicleType]) {
                                  simpleFlatRate = parseFloat(parsedData[tech?.VehicleTechnician?.payVehicleType] || 0);
                                }
                              } catch (error) {
                                console.error('Error parsing simpleFlatRate:', error);
                                simpleFlatRate = 0;
                              ...
                              // Parse amountPercentage for this technician
                              const amountPercentage = Number(tech?.VehicleTechnician?.amountPercentage);

                              // Display based on simpleFlatRate or amountPercentage
                              if ((isNaN(simpleFlatRate) || simpleFlatRate === 0) && (isNaN(amountPercentage) || amountPercentage === 0)) {
                                return <div style={{ display: 'flex', alignItems: 'center', gap: '5px', color: 'red' }}>Per Job</div>;
                              }
                              ...
                              if (!isNaN(amountPercentage) && amountPercentage > 0) {
                                ...
                                return (
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                                    ${percentageAmount.toFixed(2)} ({amountPercentage}% of Sub Total)
                                  </div>
                                );
                              }
                        ...
                        <div className="mb-2 text-sm items-center flex">
                          <strong className="mr-3 inline-block w-[180px]">Total Cost: </strong>
                          ${calculateTechnicianTotalCost(job, tech).toFixed(2)}
                        </div>
                        <div className="mb-2 text-sm items-center flex">
                          <strong className="mr-3 inline-block capitalize w-[180px]">Pay Rate:</strong>

                          <span key={index} className="mr-2">{tech.payRate}</span>

                        </div>

                        <div className="text-sm items-center flex">
                          <strong className="mr-3 inline-block w-[180px]">Work Order Status:</strong>
                          <span
                            className={`badge ${job.vehicleStatus
                              ? "badge-success bg-[#E6F9DD] text-[#1A932E] p-1 pl-4 pr-4 rounded shadow"
                              : "badge-error bg-[#FFE4E1] text-[#FF0000] p-1 pl-4 pr-4 rounded shadow"
                              }`}
                          >
                            {job.vehicleStatus ? "Completed" : "Inprogress"}
                          </span>
                        </div>
                    ...
                    <p className="text-[14px] flex items-start gap-1 mb-3 pb-3 bg-white pl-4 pr-4 border-t border-gray-500 pt-3">
                      <strong className="mr-3 pl-1 inline-block">Description:</strong>
                      {job?.jobDescription && Array.isArray(job.jobDescription) ? (
                        <ul className="list-none">
                          {job.jobDescription.map((item: string | object, index: number) => {
                            let parsedItem;
        ...
        <div className="rightsidebar shadow-lg">
          ...
          <div className="mb-5">
            <div className="flex gap-2 mb-4">
              {jobData.customer?.image ? (
                ...
                <div
                  className="w-[40px] h-[40px] rounded-full bg-[#383D71] flex items-center justify-center text-white font-bold"
                >
                  {jobData.customer?.fullName?.charAt(0)?.toUpperCase() || 'C'}
                </div>
              )}
              <div>
                <h2 className="font-[600] text-[#383D71]">{jobData.customer.fullName}</h2>
                <p className="text-[12px] text-[#383D71]">Customer</p>
              </div>
            </div>
            <p className="text-[14px] flex items-center gap-1 mb-2">
              ...
              <a className="hover:underline text-[#383D71]" href={`tel:${jobData.customer?.phoneNumber}`}>{jobData.customer?.phoneNumber || 'N/A'}</a>
            </p>
            ...
            <p className="text-[14px] flex items-center gap-1 mb-2">
              ...
              <a className="hover:underline text-[#383D71]" style={{ wordBreak: 'break-all' }} href={`mailto:${jobData.customer?.email}`}>{jobData.customer?.email || 'N/A'}</a>
            </p>
          </div>

          <h2 className="text-[18px] font-[600] mb-5 text-[#383D71]">Vehicle Details</h2>
          {jobData.vehicles[0] && (
            <div key={jobData.id} className="mb-5">
              <p className="text-[14px] flex items-center gap-1 mb-2 text-[#383D71]">
                <svg width="30" height="30" style={{ minWidth: '30px' }} viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <circle cx="17" cy="17" r="17" fill="#EDEEFF" />
                  <g clipPath="url(#clip0_2878_3030)">
