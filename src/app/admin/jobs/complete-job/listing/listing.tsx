    ...
    return (
      <React.Fragment key={completejob.id}>
        <tr key={completejob.id}>
          ...
          <td> <Link href={`/admin/jobs/view?jobId=${completejob.id}&completedJob`} className='hover:underline'>{completejob.id}</Link></td>
          <td> <Link href={`/admin/jobs/view?jobId=${completejob.id}&completedJob`} className='hover:underline capitalize'>{completejob?.customer?.firstName} {completejob?.customer?.lastName}</Link></td>

          {/* <td>{completejob?.customer?.firstName} {completejob?.customer?.lastName}</td> */}
          <td> <Link href={`/admin/technicians/view?technicianId=${completejob.technicians?.map((tech: any) => tech.id).join(',')}`} className='hover:underline'>{completejob?.technicians?.map((tech: any, index: number) => (
          ...
          <td>
            <TableActions
              editRoute={`/admin/jobs/create-job/create?jobId=${completejob.id}&completeOrder`}
              deleteRoute={`/api/deleteJob`}  // Pass the correct endpoint 
              viewRoute={`/admin/jobs/view?jobId=${completejob.id}&completedJob`}
              idKey="jobid"
