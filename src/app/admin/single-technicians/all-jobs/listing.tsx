  ...
  const toggleApproval = async (jobId: number, currentApprovalStatus: boolean) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await axios.post(`${BASE_PATH}/api/updateJobStatus`, {
          jobId,
  ...
  const renderRow = (job: any, index: number) => {
    ...
    return (
      <tr key={job.id}>
        ...
        <td>{serialNo}</td>
        {/* <td> <Link href={`/admin/jobs/view?jobId=${job.id}&workorder`} className='hover:underline'>{job.id}</Link></td> */}
        <td> {job?.jobName}</td>
        ...
        <td>
          ...
          <Link href={`/admin/jobs/view?jobId=${job.id}&workorder`} >
          ...
          {/* <TableActions
            ...
            viewRoute={`/admin/jobs/view?jobId=${job.id}&ActiveWorkOrder`}
            idKey="jobId"
            userRole='Activejobs'
