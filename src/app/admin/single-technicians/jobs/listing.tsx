...
const JobTable: React.FC = () => {
  ...
  const renderRow = (job: any) => {
    ...
    return (
      <tr key={job.id}>
        ...
        <td>{job.serialNo}</td>
        {/* <td> <Link href={`/admin/jobs/view?jobId=${job.jobId}&singleWorkOrder`} className='hover:underline'>{job.id}</Link></td> */}
        <td>{job?.jobName}</td>


        <td> <Link href={`/admin/jobs/view?jobId=${job.jobId}&singleWorkOrder`} className='hover:underline capitalize'>{job?.customer?.fullName}</Link></td>
        ...
        <td className='text-left'>
          ...
          <Link href={`/admin/reporting/view?vehicleId=${job.id}&singleWorkOrder`} >
  ...
  return (
    <div className={` mobile_listing mx-auto mt-4 transition-all duration-300 ${isCollapsed ? 'w-full pl-[5rem]' : 'container'}`}>
      <Breadcrumb
        items={[
          { label: 'All Work Order List', href: '/admin/vehicle/listing' }
        ]}
      />
