...
export default function ViewDetails() {
  ...
  const handleAccountStatusChange = async (techId: number, accountStatus: boolean) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${BASE_PATH}/api/updateTechnicianAccountStatus`,
        {
          technicianId: techId,
  ...
  const handleApprovalChange = async (techId: number, isApproved: string, tech: any) => {
    try {
      ...
      await axios.post(
        `${BASE_PATH}/api/technicianActiveUnactiveAccount`,
        {
          technicianId: techId,
                  ...
                  technicianJobs.map((job: any, index: number) => (
                    <tr key={job.id ?? index} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-6 py-4 text-right">
                        <Link
                          href={`/admin/jobs/view?jobId=${job.id}`}
                ...
                {vehiclesLoading ? (
                  ...
                  technicianVehicles.map((vehicle: any, index: number) => (
                    <tr key={vehicle.id ?? index} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-6 py-4 text-right">
                        <Link
                          href={`/admin/vehicle/view?vehicleId=${vehicle.id}`}
                          className="inline-flex items-center justify-center w-9 h-9 rounded-full transition-colors"
