  ...
  const renderRow = (cust: any) => {
    ...
    return (
      <tr key={cust.id}>
        ...
        <td>{cust.id}</td>
        <td><Link className="hover:underline" href={`/admin/customer/view?customerId=${cust.id}&allTrtCustomer`}>{cust.fullName || 'N/A'}  </Link></td>
        ...
        <td> 
          <Link href={`/admin/customer/view?customerId=${cust.id}&allTrtCustomer`}>
