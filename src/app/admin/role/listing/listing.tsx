  ...
  const handleDeleteSuccess = (deletedId: string) => {
    ...
    const fetchRoles = async (page = 1, query = '') => {
      ...
      try {
        ...
        ? `${apiUrl}/roles/searchRoles?searchQuery=${encodeURIComponent(query)}`
        : `${apiUrl}/getRoles?page=${page}`;
...
const downloadCSV = () => {
  ...
  const renderRow = (role: any) => (
    <tr key={role.id}>
      ...
      <td>
        <TableActions   
          editRoute={`/admin/role/create?roleId=${role.id}`}   
         deleteRoute={`${apiUrl}/roles/deleteRole`}  // Pass the correct endpoint
         viewRoute={`/admin/role/view?roleId=${role.id}`}
           idKey="roleId"
           userRole='Roles'
  ...
  return (
    <div className="container mx-auto mt-4">
      <CommonHeader heading="Roles" onSearch={(term) => setSearchTerm(term)}  onExport={downloadCSV} userRole='Role' buttonLabel="Create role" buttonLink="/admin/role/create" />
