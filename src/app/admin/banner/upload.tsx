...
export default function Technicians() {
  ...
  const fetchBannerData = async () => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_PATH}/api/fetchBanner`, {
        method: 'GET',
        headers,
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.bannerId) {
      formDataObj.append('id', formData.bannerId);
    }
    ...
    Promise.all(compressions)
      .then(compressedFiles => {
        setFormData((prev: any) => ({
          ...prev,
          bannerImages: [...prev.bannerImages, ...compressedFiles],
        }));
      })
      .catch(error => {
  ...
  const handleRemoveFile = async (bannerId: string | undefined, imageUrl: string | File) => {
    ...
    try {
      if (imageUrl instanceof File) {
        ...
        const response = await fetch(`${BASE_PATH}/api/removeBannerImage?bannerId=${encodeURIComponent(bannerId)}&imageUrl=${encodeURIComponent(imageUrl)}`, {
          method: 'DELETE',
          headers: {
            "Authorization": `Bearer ${token}`,
          },
      ...
      <div className='bg-white p-4 mt-5 w-[80%] m-auto'>
        <form className="" onSubmit={handleSubmit}>
          <div className='mb-4 mt-4'>
            <div className={`form-control w-full p-8 mt-1 rounded relative`} style={{ border: '2px dashed #ccc' }}>
