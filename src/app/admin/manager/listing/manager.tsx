  ...
  const fetchTechnicians = async (page = 1, query = '', limit = pageSize) => {
    ...
    try {
      ...
      const endpoint = query.trim()
        ? `/api/manager?searchQuery=${encodeURIComponent(query)}&roleType=${encodeURIComponent(roleType)}`
        : `/api/manager?page=${page}&limit=${limit}&roleType=${encodeURIComponent(roleType)}`;
  ...
  const renderRow = (tech: any, index?: number) => {
    ...
    return (
      <tr key={tech.id}>
        ...
        <td>{serialNo}</td>
        {/* <td><Link href={`/admin/technicians/view?technicianId=${tech.id}`} className='hover:underline capitalize'>{tech.id}</Link></td> */}
        <td>
          <div className="flex items-center gap-2">
            ...
            <Link href={`/admin/technicians/view?technicianId=${tech.id}`} className='hover:underline capitalize'>
        ...
        <td>
          <TableActions
            editRoute={`/admin/manager/create-manager?technicianId=${tech.id}&manager`}
            viewRoute={`/admin/manager/view?technicianId=${tech.id}`}
