...
export default function ViewDetails() {
  ...
  const fetchCustomerData = async (id: string) => {
    try {
      ...
      const response = await fetch(`${BASE_PATH}/api/fetchSingleCustomer`, {
        method: 'POST',
        headers: {
  ...
  const fetchCustomerJobsList = async (id: string, page = 1) => {
    ...
    try {
      ...
      const res = await fetch(
        `${BASE_PATH}/api/fetchCustomerJobs?customerId=${encodeURIComponent(id)}&page=${page}&limit=${LIST_PAGE_SIZE}`,
        { method: 'GET', headers }
      );
  ...
  const fetchCustomerVehiclesList = async (id: string, page = 1) => {
    ...
    try {
      ...
      const res = await fetch(
        `${BASE_PATH}/api/fetchCustomerVehicles?customerId=${encodeURIComponent(id)}&page=${page}&limit=${LIST_PAGE_SIZE}`,
        { method: 'GET', headers }
      );
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
                  ...
                  customerJobs.map((job: any, index: number) => (
                    <tr key={job.id ?? index} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-6 py-4 text-right">
                        <Link href={`/admin/jobs/view?jobId=${job.id}&ActiveWorkOrder`} className="inline-flex items-center justify-center w-9 h-9 rounded-full text-[#383d71] transition-colors" data-tooltip-id="view-job" data-tooltip-content="View">
                ...
                {vehiclesLoading ? (
                  ...
                  customerVehicles.map((vehicle: any, index: number) => (
                    <tr key={vehicle.id ?? index} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-6 py-4 text-right">
                        <Link href={`/admin/vehicle/view?vehicleId=${vehicle.id}`} className="inline-flex items-center justify-center w-9 h-9 rounded-full transition-colors" data-tooltip-id="view-vehicle" data-tooltip-content="View">
                          <Image alt="View" src={Eye} className="w-4 h-4" />
                        </Link>
                      </td>
                    </tr>
