      ...
      <tr key={cust.id}>
        ...
        <td>
          <div className="flex items-center gap-2">
            ...
            <Link href={`/admin/customer/view?customerId=${cust.id}`} className='hover:underline capitalize'>
              {cust?.fullName}
            </Link>
          </div>
        </td>
       ...
       <td>{cust.address ? cust.address.replace(/^,\s*/g, '').replace(/\s*,\s*/g, ', ') : 'N/A'}</td>
        ...
        <td className="admin-col-action">
          <TableActions
            editRoute={`/admin/customer/create?customerId=${cust.id}`}
            deleteRoute={`/api/deleteCustomer`}
            viewRoute={`/admin/customer/view?customerId=${cust.id}`}
            idKey="customerId"
            userRole="Customer"
