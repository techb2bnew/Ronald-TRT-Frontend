...
const JobTable: React.FC = () => {
  ...
  const fetchJobs = async (page = 1, query = '', limit = pageSize, customerId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        const fetchedJobs: Jobs[] = query.trim() ? data.data.vehicles || [] : data.vehicles || [];
  ...
  const renderRow = (job: any) => {
    ...
    return (
      <tr key={job.id}>
        ...
        <td> <Link href={`/admin/jobs/view?jobId=${job.id}&ActiveWorkOrder`} className='hover:underline'>{job.id}</Link></td>
        <td>{job?.jobName}</td>
        <td>{job?.vin}</td>
        <td>{job?.make}</td>
        <td>{job?.model}</td>
  ...
  return (
    <div className={`mobile_listing mobile_listing mx-auto mt-4 transition-all duration-300 ${isCollapsed ? 'w-full pl-[5rem]' : 'container'}`}>
      {/* <Breadcrumb
        items={[
          { label: 'Customer Vehicles Info', href: '/admin/jobs/active-job' }
        ]}
      /> */}
