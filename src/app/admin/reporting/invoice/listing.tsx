...
const JobTable: React.FC = () => {
  ...
  const fetchJobs = async (page = 1, query = '', limit = pageSize) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.error === 'Invalid Token') router.push('/admin');
      ...
      Papa.parse(text, {
        ...
        complete: async (result) => {
          ...
          try {
            ...
            await axios.post(`${BASE_PATH}/api/importVehicle`, { data: payloadData }, { headers });
  ...
  const fetchCustomerData = async (customerId: string) => {
    try {
      ...
      const headers: Record<string, string> = { 'Content-Type': 'application/json' };
      if (token) headers['Authorization'] = `Bearer ${token}`;
      const response = await fetch(`${BASE_PATH}/api/customerJobNamefetch?customerId=${encodeURIComponent(customerId)}`, { method: 'GET', headers });
      const data = await response.json();
      if (response.ok) {
    ...
    try {
      ...
      if (isCustomerEmailMissing) {
        ...
        if (result.isConfirmed && customerIdForEdit) {
          router.push(`/admin/customer/create?customerId=${encodeURIComponent(customerIdForEdit)}`);
        }
        return;
      }
  ...
  const renderRow = (job: any) => {
    ...
    return (
      <tr key={job.id}>
        ...
        <td>{job?.serialNo}</td>
        <td><Link href={`/admin/jobs/view?jobId=${job?.job?.id}&ActiveWorkOrder`} className='hover:underline'>{job?.jobName}</Link></td>
        <td><Link href={`/admin/vehicle/view?vehicleId=${job.id}`} className='hover:underline'>{job?.vin}</Link></td>
        <td>{formatStockNumberCell(job)}</td>
        <td><Link href={`/admin/client/view?customerId=${job?.customer?.id}`} className='hover:underline'>{job?.customer?.fullName}</Link></td>
            ...
            {job?.assignedTechnicians
              ...
              ?.map((tech: any) => (
                <div key={tech.id} className="flex items-center gap-2 mb-1">
                  ...
                  <Link
                    href={`/admin/technicians/view?technicianId=${tech?.id}`}
        ...
        <td className='text-left'>
          <div className="flex gap-3 items-center">
            <Link href={`/admin/vehicle/view?vehicleId=${job.id}`}>
