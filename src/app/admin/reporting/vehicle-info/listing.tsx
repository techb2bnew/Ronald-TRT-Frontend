  ...
  const renderRow = (job: any) => {
    ...
    return (
      <tr key={job.id}>
        ...
        <td> <Link href={`/admin/reporting/view?vehicleId=${job.id}`} className='hover:underline'> {job?.id}</Link> </td>
        <td>{job.customer.fullName}</td>
        <td> <a className="hover:underline" href={`mailto:${job?.customer.email}`}>{job.customer.email || 'N/A'}</a></td>
        <td>{job.vin}</td>

        {/* <td> <Link href={`/admin/reporting/view?vehicalId=${job.vehicalId}&completedJob`} className='hover:underline capitalize'>{job?.customer?.firstName} {job?.customer?.lastName}</Link></td>
        ...
        <td>
          {/* <TableActions
            editRoute={`/admin/jobs/create-job/create?jobId=${job.id}&vehicleInfo`}
            deleteRoute={`${apiUrl}/deleteJobs`}  // Pass the correct endpoint
            viewRoute={`/admin/reporting/view?vehicalId=${job.vehicalId}`}
            idKey="jobid"
          ...
          <Link href={`/admin/reporting/view?vehicleId=${job.id}`} >
