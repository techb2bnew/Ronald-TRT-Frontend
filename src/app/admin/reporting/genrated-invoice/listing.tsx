...
const JobTable: React.FC = () => {
  ...
  const fetchJobs = async (page = 1, query = '', limit = pageSize) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const jobsWithSerial = withSerialNo(fetchedTechnicians, (page - 1) * limit + 1);
  ...
  const handleNewJobClick = async (jobId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        setActiveJob(withSerialNo(data.invoice.invoices || []));
  ...
  useEffect(() => {
    if (selectedStatus === '') {
      setActiveJob(withSerialNo(originalJobs));
    } else {
  ...
  const renderRow = (job: any) => {
    ...
    return (
      <tr key={job.id}>
        ...
        <td>{job?.serialNo}</td>
        <td> <Link href={`/admin/reporting/view-invoice?invoiceId=${job.invoiceNumber}`} className='hover:underline'> {job?.invoiceNumber}</Link> </td>
        ...
        <td className='text-left'>
          <div className="flex gap-3 items-center">
            ...
            <Link href={`/admin/reporting/view-invoice?invoiceId=${job.invoiceNumber}`} >
