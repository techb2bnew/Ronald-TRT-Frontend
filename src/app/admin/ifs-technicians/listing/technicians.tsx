  ...
  const fetchTechnicians = async (page = 1, query = '') => {
    ...
    try {
      ...
      const endpoint = query.trim()
        ? `${apiUrl}/searchTechnicians?searchQuery=${encodeURIComponent(query)}&types=ifs`
        : `${apiUrl}/fetchTechnician?page=${page}&types=ifs`;
 ...
 const status = statuses[tech.id] || "Accept"; 
    ...
    <tr key={tech.id}>
      ...
      {/* <td>{tech.payRate}</td> */}
       <td onClick={() => handleStatusChange(tech.id, status)} style={{ cursor: 'pointer' }}>
        ...
        <Link href='/admin/jobs/create-job/create' className='flex gap-1 items-center border border-black rounded p-2 pl-2 pr-2 w-[120px] justify-center'>Create Job
      ...
      <td> 
      <TableActions
          editRoute={`/admin/technicians/create-technician?technicianId=${tech.id}&singletechnician`}
          viewRoute={`/admin/ifs-technicians/view?technicianId=${tech.id}`}
        ...
        />
         {/* <Link className="p-1" href={`/admin/single-technicians/view?technicianId=${tech.id}`}>
  ...
  return (
    <div className="container mx-auto mt-4">
      <CommonHeader heading="IFS Technicians" onSearch={(term) => setSearchTerm(term)}  onExport={downloadCSV} userRole='SingleTechnician'  buttonLabel="Create Ifs Technician" buttonLink="/admin/technicians/create-technician?singletechnician" />
