...
//                                         <input type="checkbox" checked={agreeTerms} onChange={e => setAgreeTerms(e.target.checked)} className="peer h-5 w-5 cursor-pointer transition-all appearance-none rounded shadow bg-white hover:shadow-md border border-slate-300 checked:bg-[#383d71] checked:border-[#383d71]" id="check" />
...
//                                     </label>
//                                 </div>
//                                 <Link href="/admin/forgot" className="text-sm primary-text">Forgot Password?</Link>
//                             </div>
...
//                                 <Link href='/admin' className="primary-text"> Login</Link> </p>
//                         </div>
...
export default function Role() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        setSubmitting(true);
        const response = await fetch(`${apiUrl}/${isEdit ? 'updateIndividualTechnician' : 'register'}`, { 
            method: 'POST',
  ...
  const fetchTechnicianData = async (technicianId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/fetchSingleIndividualTechnician?technicianId=${technicianId}`, {
        method: 'GET',
        headers,
      });
      if (response.status == 400) {
  ...
  return (
    <div className='main-container mb-5'>
      ...
      <h1 className="text-lg leading-6 font-bold text-gray-900 mb-[2px] sm:mb-0">{isEdit ? 'Edit Technician' : 'Create Single Technician'}</h1>
          <div className="text-center mb-5 w-full"> 
          </div>
