 ...
 const fetchArchive = async (page = 1, query = '', limit = pageSize) => {
    ...
    if (roleType !== 'superadmin') {
      switch (selectedArchiveType) {
        ...
        case ARCHIVE_TYPES.JOB:
          endpoint = `/api/archiveIfs?${params}&userId=${encodeURIComponent(userID || '')}`;
          break;
        default:
          endpoint = `/api/archiveIfs?${params}&userId=${encodeURIComponent(userID || '')}`;
      }
  ...
  const handleRecoverRecord = async (id: number, type: string) => {
    try {
      ...
      const response = await fetch(`${BASE_PATH}/api/recoverRecords`, {
        method: "POST",
        headers,
      ...
      case ARCHIVE_TYPES.JOB:
        return (
          <>
            ...
            <td className='capitalize'>{item?.customer?.firstName} {item?.customer?.lastName}</td>
  ...
  return (
    <div className={`mobile_listing mx-auto mt-4 transition-all duration-300 ${isCollapsed ? 'w-full pl-[5rem]' : 'container'}`}>
      ...
      <Breadcrumb
        items={[
          { label: 'IFS Archive', href: '/admin/archive/listing' }
        ]}
      />
