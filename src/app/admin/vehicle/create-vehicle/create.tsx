...
export default function Technicians() {
  ...
  const handleAddVehicle = async (submit: boolean = false) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (submit) {
          router.push('/admin/vehicle/listing');
        }
        if (isEdit) {
          router.push('/admin/vehicle/listing');
        }
  ...
  const fetchCustomerJobsPage = async (customerId: string, page: number, append: boolean) => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_PATH}/api/customerJobNamefetch?${params.toString()}`, {
        method: 'GET',
        headers,
      });
  ...
  const fetchVehicleDetails = async (vin: string, index: number) => {
    ...
    try {
      ...
      if (vehicleDetailsResponse?.ok && vehicleDetailsData?.Results) {
        ...
        const fallbackRes = await fetch(`${BASE_PATH}/api/decodeVinFallback?vin=${encodeURIComponent(vin)}`, {
          method: 'GET',
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.error === 'Invalid Token') {
          router.push('/admin'); // Redirect if the token is invalid
        } else {
