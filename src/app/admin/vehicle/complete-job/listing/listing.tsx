  ...
  const toggleApproval = async (vehicleId: number, currentApprovalStatus: boolean) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await axios.post(`${BASE_PATH}/api/workOrderComplete`, {
          vehicleId,
          completedBy,
    ...
    return (
      <>
        <tr key={completejob.id}>
          ...
          <td> <Link href={`/admin/vehicle/view?vehicleId=${completejob.id}`} className='hover:underline'> {completejob.id}</Link> </td>
          ...
          <td>{completejob.startDate ? formatDisplayDate(completejob.startDate) : ''}</td>
          <td>{completejob.endDate ? formatDisplayDate(completejob.endDate) : ''}</td>
          ...
          <td>
            <TableActions
              editRoute={`/admin/vehicle/create-vehicle?vahicleId=${completejob.id}`}
              deleteRoute={`/api/deleteVehicle`}  // Pass the correct endpoint
              viewRoute={`/admin/vehicle/view?vehicleId=${completejob.id}`}
              idKey="vehicleId"
              userRole='Activejobs'
