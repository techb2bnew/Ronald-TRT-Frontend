  ...
  const hydrateFilterVehiclesPay = async (vehicles: any[], token: string) => {
    ...
    return Promise.all(
      vehicles.map(async (vehicle) => {
        try {
          const res = await fetch(`${BASE_PATH}/api/fetchSingleVehicleInfo?vehicleId=${vehicle.id}`, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
  ...
  const renderRow = (job: any, index: number) => {
    ...
    return (
      <tr key={job.id}>
        ...
        <td>{serialNo}</td>
        {/* <td><Link href={`/admin/vehicle/view?vehicleId=${job.id}`} className='hover:underline'>{job.id}</Link></td> */}
        <td>{job?.jobName}</td>
        ...
        <td>
          <TableActions
            editRoute={`/admin/vehicle/create-vehicle?vahicleId=${job.id}`}
            deleteRoute={`/api/deleteVehicle`}
            viewRoute={`/admin/vehicle/view?vehicleId=${job.id}`}
            idKey="vehicleId"
            userRole='Activejobs'
            itemId={job.id}
