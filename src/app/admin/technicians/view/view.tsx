...
export default function ViewDetails() {
  ...
  const fetchTechnicianJobsList = async (techId: string, page = 1) => {
    ...
    try {
      ...
      const res = await fetch(
        `${BASE_PATH}/api/fetchTechnicianJobs?technicianId=${encodeURIComponent(techId)}&page=${page}&limit=${LIST_PAGE_SIZE}`,
        { method: 'GET', headers }
      );
  ...
  const fetchTechnicianVehiclesList = async (techId: string, page = 1) => {
    ...
    try {
      ...
      const res = await fetch(
        `${BASE_PATH}/api/fetchTechnicianVehicles?technicianId=${encodeURIComponent(techId)}&page=${page}&limit=${LIST_PAGE_SIZE}`,
        { method: 'GET', headers }
      );
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
      ...
      <div className="mx-auto">
        {/* <div className="flex items-center gap-3 mb-4">
          <Link href={backHref} className="flex items-center gap-2 hover:opacity-90 transition-opacity">
            ...
            <span className="font-semibold text-lg">Technician Detail</span>
          </Link>
        </div> */}
        ...
        <div className="  overflow-hidden mt-4 pb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 ">
            ...
            <InfoCard
              ...
              label="Type"
              value={<span className="capitalize">{technician?.techType === 'technician' ? 'Dent Tech' : technician?.techType || 'N/A'}</span>}
            />
            <InfoCard
            ...
            <InfoCard
              ...
              value={
                <div
                  ...
                  style={{ cursor: technician.isApproved === 'accept' || technician.accountStatus ? 'pointer' : 'not-allowed' }}
                >
                ...
                technician?.taxForms?.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {technician.taxForms.map((form: any, index: number) => {
                      const isPDF = typeof form === 'string' && form.endsWith('.pdf');
                      return (
                        <span key={index}>
                          {isPDF ? (
                            <button type="button" onClick={() => window.open(form, '_blank')} className="text-[#383d71] hover:underline">
                              Technician Tax Form {index + 1}
                            </button>
                          ) : (
                            <button type="button" onClick={() => setPreviewImage(form)} className="text-[#383d71] hover:underline">
                              Technician Tax Form {index + 1}
                            </button>
                          )}
                        </span>
                  ...
                  technicianJobs.map((job: any, index: number) => (
                    <tr key={job.id ?? index} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-6 py-4 text-right">
                        <Link href={`/admin/jobs/view?jobId=${job.id}&ActiveWorkOrder`} className="inline-flex items-center justify-center w-9 h-9  " data-tooltip-id="view-job" data-tooltip-content="View">
                ...
                {vehiclesLoading ? (
                  ...
                  technicianVehicles.map((vehicle: any, index: number) => (
                    <tr key={vehicle.id ?? index} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-6 py-4 text-right">
                        <Link href={`/admin/vehicle/view?vehicleId=${vehicle.id}`} className="inline-flex items-center justify-center w-9 h-9  " data-tooltip-id="view-vehicle" data-tooltip-content="View">
                          <Image alt="View" src={Eye} className="w-4 h-4  " />
                        </Link>
                      </td>
                    </tr>
