...
export default function ClientListing() {
  ...
  const fetchCustomer = async (page = 1, query = '', limit = pageSize) => {
    ...
    try {
      ...
      const endpoint = query.trim()
        ? `${apiUrl}/searchIndividualTechnicianCustomers?searchQuery=${encodeURIComponent(query)}&roleType=single-technician`
  ...
  return (
    <div className={` mx-auto mt-4 transition-all duration-300 ${isCollapsed ? 'w-full pl-[5rem]' : 'container'}`}>
      ...
      />
      <CommonHeader heading='All Customer' onPageSizeChange={handlePageSizeChange} onSearch={(term) => setSearchTerm(term)} onExport={downloadCSV} userRole='' buttonLabel="" buttonLink="" />
